...
cargo_root="$repo_root/opencode-rs"
...
(
  cd "$cargo_root"
  cargo run -p opencode-cli --bin opencode -- exec-server --listen "$listen_url"
...
cd "$cargo_root"
cargo run -p codex-tui --bin opencode-tui -- -c mcp_oauth_credentials_store=file "$@"
