...
#[derive(Debug, Parser)]
#[command(name = "opencode-state-logs")]
#[command(about = "Tail OpenCode logs from the dedicated logs SQLite DB with simple filters")]
struct Args {
    /// Path to OPENCODE_HOME. Defaults to $OPENCODE_HOME, then $CODEX_HOME, then ~/.opencode.
    #[arg(long = "opencode-home", alias = "codex-home", env = "OPENCODE_HOME")]
    opencode_home: Option<PathBuf>,

    /// Direct path to the logs SQLite database. Overrides --opencode-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
mod tests {
    ...
    fn log_level_rejects_aliases_and_unknown_values() {
        assert!(Args::try_parse_from(["opencode-state-logs", "--level", "warning"]).is_err());
        assert!(Args::try_parse_from(["opencode-state-logs", "--level", "err"]).is_err());
        assert!(Args::try_parse_from(["opencode-state-logs", "--level", "warn,error"]).is_err());
    }

    #[test]
    fn log_level_accepts_canonical_values_case_insensitively() {
        let args = Args::try_parse_from(["opencode-state-logs", "--level", "WARN"])
