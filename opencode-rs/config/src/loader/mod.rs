...
#[cfg(unix)]
const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/opencode/config.toml";
#[cfg(unix)]
const LEGACY_SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/codex/config.toml";
#[cfg(unix)]
const SYSTEM_REQUIREMENTS_TOML_FILE_UNIX: &str = "/etc/opencode/requirements.toml";
#[cfg(unix)]
const LEGACY_SYSTEM_REQUIREMENTS_TOML_FILE_UNIX: &str = "/etc/codex/requirements.toml";

const PROJECT_CONFIG_DIR_NAME_PRIMARY: &str = ".opencode";
const PROJECT_CONFIG_DIR_NAME_LEGACY: &str = ".codex";
...
/// - system    `/etc/opencode/requirements.toml` (Unix) or
...
///
/// - admin:    managed preferences (*)
/// - system    `/etc/opencode/config.toml` (Unix) or
///   `%ProgramData%\OpenAI\Codex\config.toml` (Windows)
/// - user      `${OPENCODE_HOME}/config.toml`
/// - profile   `${OPENCODE_HOME}/<name>.config.toml`, when selected
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.opencode/config.toml` (loaded but disabled when the directory is untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.opencode/config.toml` (loaded but disabled when untrusted)
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let primary = Path::new(SYSTEM_REQUIREMENTS_TOML_FILE_UNIX);
    let selected = if primary.exists() {
        primary
    } else {
        Path::new(LEGACY_SYSTEM_REQUIREMENTS_TOML_FILE_UNIX)
    };
...
pub fn system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let primary = Path::new(SYSTEM_CONFIG_TOML_FILE_UNIX);
    let selected = if primary.exists() {
        primary
    } else {
        Path::new(LEGACY_SYSTEM_CONFIG_TOML_FILE_UNIX)
    };
    AbsolutePathBuf::from_absolute_path(selected)
}
...
fn windows_codex_system_dir() -> PathBuf {
    ...
    let primary = program_data.join("OpenAI").join("OpenCode");
    if primary.exists() {
        primary
    } else {
...
impl ProjectTrustContext {
    ...
    fn root_checkout_hooks_folder_for_dir(&self, dir: &AbsolutePathBuf) -> Option<AbsolutePathBuf> {
        ...
        Some(repo_root.join(relative_dir).join(PROJECT_CONFIG_DIR_NAME_PRIMARY))
    }
}
...
async fn load_project_layers(
    ...
    for dir in dirs {
        ...
        for config_dir_name in [
            PROJECT_CONFIG_DIR_NAME_PRIMARY,
            PROJECT_CONFIG_DIR_NAME_LEGACY,
        ] {
            let candidate = dir.join(config_dir_name);
            if fs
