...
async fn responses_mode_stream_cli() {
    ...
    let bin = codex_utils_cargo_bin::cargo_bin("opencode").unwrap();
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    let bin2 = codex_utils_cargo_bin::cargo_bin("opencode").unwrap();
    ...
    assert!(output2.status.success(), "resume opencode-cli run failed");
