...
pub struct Config {
    ...
    /// event.
    ///
    /// Example `~/.opencode/config.toml` snippet:
    ///
    /// ```toml
    ...
    ///          https://github.com/openai/codex/blob/main/opencode-rs/rmcp-client/src/oauth.rs#L2
    /// file: OPENCODE_HOME/.credentials.json
    ...
    /// Directory containing all OpenCode state (defaults to `~/.opencode` but can be
    /// overridden by the `OPENCODE_HOME` environment variable).
    pub codex_home: AbsolutePathBuf,

    /// Directory where Codex stores the SQLite state DB.
    pub sqlite_home: PathBuf,

    /// Directory where OpenCode writes log files (defaults to `$OPENCODE_HOME/log`).
    pub log_dir: PathBuf,
    ...
    /// Settings that govern if and what will be written to `~/.opencode/history.jsonl`.
    pub history: History,
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .opencode/ folders.
...
/// Patch `OPENCODE_HOME/config.toml` project state to set trust level.
/// Use with caution.
...
/// `CODEX_HOME`). If not set, defaults to `~/.opencode`.
///
