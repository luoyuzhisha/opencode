...
fn codex_binary(repo_root: &Path) -> Result<PathBuf> {
    if let Ok(path) = codex_utils_cargo_bin::cargo_bin("opencode") {
        return Ok(path);
    }

    let fallback = repo_root.join("opencode-rs/target/debug/codex");
    anyhow::ensure!(
        fallback.is_file(),
        "codex binary is unavailable; run `cargo build -p opencode-cli` first"
    );
    Ok(fallback)
}
