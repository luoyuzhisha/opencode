# Bazel in opencode-rs

This repository uses Bazel to build the Rust workspace under `opencode-rs`.
...
- `rules_rs` imports third-party crates from `opencode-rs/Cargo.toml` and
  `opencode-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `opencode-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
