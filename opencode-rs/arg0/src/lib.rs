...
pub struct Arg0DispatchPaths {
    /// Stable path to the current OpenCode executable for child re-execs.
    ///
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    match prepend_path_entry_for_codex_aliases() {
        ...
        Err(err) => {
            // It is possible that OpenCode will proceed successfully even if
...
/// While we want to deploy the OpenCode CLI as a single executable for simplicity,
...
/// `opencode-linux-sandbox` we *directly* execute
/// [`codex_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.opencode/.env` before creating any threads.
/// 2.  Construct a Tokio multi-thread runtime.
/// 3.  Capture the current executable path and derive the
///     `opencode-linux-sandbox` helper path (falling back to the current
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `opencode-linux-sandbox` alias when available so callers can
...
/// Load env vars from ~/.opencode/.env.
///
...
///   with the hidden `--opencode-run-as-apply-patch` flag.
///
...
mod tests {
    ...
    fn linux_sandbox_exe_path_prefers_codex_linux_sandbox_alias() -> std::io::Result<()> {
        ...
        let alias_path = temp_dir.path().join("opencode-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                codex_self_exe: Some(PathBuf::from("/usr/bin/opencode")),
        ...
        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/opencode"))),
            Some(alias_path),
        );
        Ok(())
    }
    ...
    fn run_main_with_arg0_guard_keeps_aliases_alive_until_main_returns() -> anyhow::Result<()> {
        let temp_dir = TempDir::new()?;
        let alias_path = temp_dir.path().join("opencode-helper-alias");
