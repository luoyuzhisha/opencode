...
Run from `<reporoot>/opencode-rs`.

```bash
# 1) Build debug codex binary
cargo build -p opencode-cli --bin opencode
...
```bash
cargo run --bin opencode-app-server-test-client -- \
...
```bash
cargo run --bin opencode-app-server-test-client -- thread-resume <THREAD_ID>
```
