...
fn ensure_supported_platform() -> Result<()> {
    Err(anyhow!(
        "opencode app-server daemon lifecycle is only supported on Unix platforms"
    ))
}
...
impl Daemon {
    ...
    async fn restart(&self) -> Result<LifecycleOutput> {
        ...
        {
            return Err(anyhow!(
                "app server is running but is not managed by opencode app-server daemon"
            ));
        }
    ...
    fn ensure_managed_codex_bin(&self) -> Result<()> {
        ...
        Err(anyhow!(
            "managed standalone OpenCode install not found at {managed_codex_path}\n\n\
             This command requires the standalone install managed by the OpenCode installer, because \
             the daemon starts and updates app-server from that fixed path.\n\n\
             Install it with:\n  curl -fsSL https://chatgpt.com/opencode/install.sh | sh\n\
             (if unavailable, use legacy URL: https://chatgpt.com/codex/install.sh)\n\n\
...
mod tests {
    ...
    fn remote_control_start_output_serializes_inner_output_without_tag() {
        let lifecycle_output = LifecycleOutput {
            ...
            pid: None,
            managed_codex_path: "opencode".into(),
            managed_codex_version: Some("1.2.3".to_string()),
            socket_path: "opencode.sock".into(),
        ...
        assert_eq!(
            ...
            serde_json::json!({
                ...
                "backend": "pid",
                "managedCodexPath": "opencode",
                "managedCodexVersion": "1.2.3",
                "socketPath": "opencode.sock",
                "cliVersion": "1.2.3",
    ...
    async fn not_ready_context_reports_daemon_app_server_before_stderr() {
        ...
        let daemon = Daemon {
            ...
            managed_codex_bin: temp_dir.path().join("missing-opencode"),
        };
