...
#[cfg(unix)]
const PRIMARY_STANDALONE_INSTALL_SCRIPT_URL: &str = "https://chatgpt.com/opencode/install.sh";
#[cfg(unix)]
const LEGACY_STANDALONE_INSTALL_SCRIPT_URL: &str = "https://chatgpt.com/codex/install.sh";
...
pub(crate) fn reexec_managed_updater(managed_codex_bin: &std::path::Path) -> Result<()> {
    ...
    Err(err).with_context(|| {
        format!(
            "failed to replace updater with managed OpenCode binary {}",
...
async fn install_latest_standalone() -> Result<()> {
    let script =
        match fetch_standalone_install_script(PRIMARY_STANDALONE_INSTALL_SCRIPT_URL).await {
            Ok(script) => script,
            Err(primary_err) => {
                fetch_standalone_install_script(LEGACY_STANDALONE_INSTALL_SCRIPT_URL)
                    .await
                    .with_context(|| {
                        format!(
                            "failed to fetch standalone OpenCode updater from \
                             {PRIMARY_STANDALONE_INSTALL_SCRIPT_URL} and fallback \
                             {LEGACY_STANDALONE_INSTALL_SCRIPT_URL}: {primary_err}"
                        )
                    })?
            }
        };
    ...
    let mut child = Command::new("/bin/sh")
        ...
        .stderr(Stdio::null())
        .spawn()
        .context("failed to invoke standalone OpenCode updater")?;
    let mut stdin = child
        .stdin
        .take()
        .context("standalone OpenCode updater stdin was unavailable")?;
    stdin
        .write_all(&script)
        .await
        .context("failed to pass standalone OpenCode updater to shell")?;
    drop(stdin);
    let status = child
        .wait()
        .await
        .context("failed to wait for standalone OpenCode updater")?;

    if status.success() {
        Ok(())
    } else {
        anyhow::bail!("standalone OpenCode updater exited with status {status}")
    }
}

#[cfg(unix)]
async fn fetch_standalone_install_script(url: &str) -> Result<Vec<u8>> {
    let bytes = reqwest::get(url)
        .await
        .with_context(|| format!("failed to fetch standalone updater from {url}"))?
        .error_for_status()
        .with_context(|| format!("standalone updater request failed for {url}"))?
        .bytes()
        .await
        .with_context(|| format!("failed to read standalone updater from {url}"))?;
    Ok(bytes.to_vec())
}
