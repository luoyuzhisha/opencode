...
#[command(
    version,
    override_usage = "opencode exec [OPTIONS] [PROMPT]\n       opencode exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Allow running OpenCode outside a Git repository.
...
struct ResumeArgsRaw {
    ...
    // so "opencode resume --last <prompt>" treats the positional as a prompt, not a session id.
