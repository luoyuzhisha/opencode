...
Typed clients for Codex/OpenAI APIs built on top of the generic transport in `opencode-client`.
...
All HTTP details (URLs, headers, retry/backoff policies, SSE framing) are encapsulated in `codex-api` and `opencode-client`. Callers construct prompts/inputs using protocol types and work with typed streams of `ResponseEvent` or compacted `ResponseItem` values.
