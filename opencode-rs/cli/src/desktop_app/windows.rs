...
pub async fn run_windows_app_open_or_install(
    ...
    if let Some(app_id) = find_codex_app_id().await? {
        eprintln!("Opening OpenCode Desktop...");
        open_installed_codex_app(&app_id).await?;
        eprintln!(
            "In OpenCode Desktop, open workspace {workspace}.",
            workspace = display_workspace_path(&workspace)
        );
        return Ok(());
    }

    eprintln!("OpenCode Desktop not found; opening Windows installer...");
    ...
    eprintln!(
        "After installing OpenCode Desktop, open workspace {workspace}.",
        workspace = display_workspace_path(&workspace)
    );
    Ok(())
}
...
async fn find_codex_app_id() -> anyhow::Result<Option<String>> {
    if let Some(app_id) = find_codex_app_id_by_name("OpenCode").await? {
        return Ok(Some(app_id));
    }
    find_codex_app_id_by_name("Codex").await
}

async fn find_codex_app_id_by_name(app_name: &str) -> anyhow::Result<Option<String>> {
    let output = Command::new("powershell.exe")
        .arg("-NoProfile")
        .arg("-Command")
        .arg(format!(
            "Get-StartApps -Name '{app_name}' | Select-Object -First 1 -ExpandProperty AppID"
        ))
        .output()
        .await
...
mod tests {
    ...
    fn display_workspace_path_removes_windows_extended_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\C:\Users\fcoury\code\opencode")),
            r"C:\Users\fcoury\code\opencode"
        );
    }
    ...
    fn display_workspace_path_preserves_unc_prefix() {
        assert_eq!(
            display_workspace_path(Path::new(r"\\?\UNC\server\share\opencode")),
            r"\\server\share\opencode"
        );
    }
