...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_codex_app_path() {
        eprintln!(
            "Opening OpenCode Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_codex_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("OpenCode Desktop not found; downloading installer...");
    ...
    let installed_app = download_and_install_codex_to_user_applications(&download_url)
        .await
        .context("failed to download/install OpenCode Desktop")?;
    eprintln!(
        "Launching OpenCode Desktop from {installed_app}...",
...
fn candidate_codex_app_paths() -> Vec<PathBuf> {
    ...
    if let Some(home) = std::env::var_os("HOME") {
        ...
        paths.push(applications_dir.join("OpenCode.app"));
        paths.push(applications_dir.join("Codex.app"));
    }
    paths
}
...
async fn download_and_install_codex_to_user_applications(dmg_url: &str) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("opencode-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("OpenCode.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting OpenCode Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_codex_app_in_mount(&mount_point)
            .context("failed to locate OpenCode.app in mounted dmg")?;
...
async fn install_codex_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing OpenCode Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("OpenCode.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install OpenCode.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install OpenCode.app to any applications directory");
}
...
fn find_codex_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("OpenCode.app");
    if direct.is_dir() {
        return Ok(direct);
    }
    let legacy = mount_point.join("Codex.app");
    if legacy.is_dir() {
        return Ok(legacy);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex\t/Volumes/OpenCode\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/OpenCode")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tCodex Installer\t/Volumes/OpenCode Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/OpenCode Installer")
        );
    }
}
