...
const PROTECTED_METADATA_OPENCODE_PATH_NAME: &str = ".opencode";
...
pub const PROTECTED_METADATA_PATH_NAMES: &[&str] = &[
    ...
    PROTECTED_METADATA_OPENCODE_PATH_NAME,
...
pub fn is_protected_metadata_directory_name(name: &OsStr) -> bool {
    name == OsStr::new(PROTECTED_METADATA_AGENTS_PATH_NAME)
        || name == OsStr::new(PROTECTED_METADATA_OPENCODE_PATH_NAME)
...
pub(crate) fn default_read_only_subpaths_for_writable_root(
    ...
    // Keep top-level project metadata under .opencode read-only to the agent by
    ...
    let top_level_opencode = writable_root.join(PROTECTED_METADATA_OPENCODE_PATH_NAME);
    if protect_missing_dot_codex || top_level_opencode.as_path().is_dir() {
        subpaths.push(top_level_opencode);
    }

    // Keep legacy .codex metadata protected when present on disk.
