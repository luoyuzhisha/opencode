...
def codex_rust_crate(
    ...
    if manifest_relpath.startswith("opencode-rs/"):
        manifest_relpath = manifest_relpath[len("opencode-rs/"):]
    ...
    if lib_srcs:
        ...
        rust_test(
            ...
            # tests under gnullvm.
            # Bazel has emitted both `opencode-rs/<crate>/...` and
            # `../opencode-rs/<crate>/...` paths for `file!()`. Strip either
            # prefix so the workspace-root launcher sees Cargo-like metadata
            # such as `tui/src/...`.
            rustc_flags = rustc_flags_extra + WINDOWS_RUSTC_LINK_FLAGS + [
                "--remap-path-prefix=../opencode-rs=",
                "--remap-path-prefix=opencode-rs=",
            ],
            rustc_env = rustc_env,
        ...
        workspace_root_test(
            ...
            workspace_root_marker = "//opencode-rs/utils/cargo-bin:repo_root.marker",
            tags = test_tags,
            **unit_test_kwargs
        )
