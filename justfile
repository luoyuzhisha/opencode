set working-directory := "opencode-rs"
...
alias c := codex
codex *args:
    cargo run --bin opencode -- "$@"

# `codex exec`
exec *args:
    cargo run --bin opencode -- exec "$@"
...
file-search *args:
    cargo run --bin opencode-file-search -- "$@"
...
[no-cd]
bazel-codex *args:
    bazel run //opencode-rs/cli:codex --run_under="cd $PWD &&" -- "$@"
...
build-for-release:
    bazel build //opencode-rs/cli:release_binaries --config=remote
...
write-config-schema:
    cargo run -p codex-core --bin opencode-write-config-schema
...
[no-cd]
write-hooks-schema:
    cargo run --manifest-path {{ justfile_directory() }}/opencode-rs/Cargo.toml -p codex-hooks --bin write_hooks_schema_fixtures

# Run the argument-comment Dylint checks across opencode-rs.
[no-cd]
